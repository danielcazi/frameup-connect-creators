-- Fix the get_creator_favorites function to use the correct rating column
-- The table reviews has 'rating_overall' instead of 'rating'
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(
      (SELECT AVG(r.rating_overall) FROM reviews r WHERE r.reviewee_id = cf.editor_id), -- Changed from r.rating to r.rating_overall
      0
