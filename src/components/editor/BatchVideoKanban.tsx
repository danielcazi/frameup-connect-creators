...
// ═══════════════════════════════════════════════════════════════════════════════
// DEFINIÇÃO DAS COLUNAS DO KANBAN DO LOTE
// ═══════════════════════════════════════════════════════════════════════════════
...
const BATCH_KANBAN_COLUMNS = [
    {
        ...
        description: 'Você ainda não começou',
        color: '#F59E0B',
...
function mapBatchVideoStatusToKanbanColumn(status: string): string {
    switch (status) {
        case 'pending':
...
const BatchVideoCard = ({ video, projectId, columnColor, basePrice }: BatchVideoCardProps) => {
    ...
    // Verificar se precisa entregar (status pending ou in_progress ou revision_requested)
    const needsDelivery = ['pending', 'in_progress', 'revision_requested'].includes(video.status);
    const isCompleted = ['completed', 'approved'].includes(video.status);
    ...
    return (
        <div
            ...
            onClick={() => navigate(`/editor/project/${projectId}?video=${video.id}`)}
        >
            ...
            <Button
                ...
                style={needsDelivery ? {} : { borderColor: `${columnColor}50`, color: columnColor }}
                onClick={(e) => {
                    e.stopPropagation();
                    navigate(`/editor/project/${projectId}?video=${video.id}`);
                }}
            >
                {needsDelivery ? (
                    <>
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 mt-1">
                                ...
                                <span className="flex items-center gap-1">
                                    ...
                                    {project.deadline_days}d restantes
                                </span>
                            </div>
                        </div>
                    </div>
